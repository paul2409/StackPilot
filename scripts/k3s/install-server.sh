...
set +e
sed 's/\r$//' "${ROOT_DIR}/scripts/k3s/server-node-setup.sh" | \
  vagrant ssh "${CONTROL_NAME}" -c "sudo env NODE_IP='${NODE_IP}' K3S_VERSION='${K3S_VERSION}' bash -s" \
...
set +e
sed 's/\r$//' "${ROOT_DIR}/scripts/k3s/debug-snapshot.sh" | \
  vagrant ssh "${CONTROL_NAME}" -c "sudo bash -s" \
  2>&1 | tee "${DEBUG_LOG}"
set -e
