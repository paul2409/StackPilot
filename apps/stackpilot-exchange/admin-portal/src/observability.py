...
ADMIN_PORTAL_UPSTREAM_FAILURES = Counter(
    "admin_portal_upstream_failures_total",
    "Admin portal upstream failures",
    ["dependency"],
)
ADMIN_PORTAL_SUMMARY_REQUESTS = Counter(
    "admin_portal_summary_requests_total",
    "Admin portal summary requests",
)
...
def record_dependency_unready(dependency: str):
    ...
    ADMIN_PORTAL_UPSTREAM_FAILURES.labels(dependency=dependency).inc()

def record_summary_request():
    ADMIN_PORTAL_SUMMARY_REQUESTS.inc()
